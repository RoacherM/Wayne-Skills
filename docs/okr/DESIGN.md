...
agent 无关的协议文档 `PROTOCOL.md`，仓库内 `skills/okr`（给对话 agent；执行 agent 侧暂无单独 skill，2026-09-04 移除；`npm install -g` 时随 CLI 一起装进 `~/.agents/skills`，也可 `npx skills add`）和 `AGENTS.okr.md`（给不支持 skill 的 agent 的片段）共用：
...
2. `PROTOCOL.md` + 仓库内 `skills/okr`（随 `npm install -g` 一起装，也可 `npx skills add`；CLI 和 skill 一起并入 Wayne-Skills 仓库，取代旧的 okr skill）+ AGENTS.md 片段。
